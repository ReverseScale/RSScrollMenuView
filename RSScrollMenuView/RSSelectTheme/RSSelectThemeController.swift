...
typealias DataSourceType = Array<Array<RSThemeDataSourceProtocal>>

typealias UpdataDataSourceClosure = (DataSourceType!) -> Void

class RSSelectThemeController: UIViewController, UICollectionViewDataSource{
    
    private var themeCollectionView: RSThemeCollectionView!
    ...
    func showAlertView() {
        let alertController = UIAlertController(title: "请至少保留一项",
    ...
    func tapCellButton(cell: RSThemeCollectionViewCell) {
    ...
    private func addCloseButton() {
        let closeButton = RSCloseButton(frame: CGRect(x: SCREEN_WIDTH - 70, y: 30, width: 50, height: 30))
    ...
    private func addThemeCollectionView() {
        self.themeCollectionView = RSThemeCollectionView(frame: CGRect(x: 0, y: 60, width:themeCollectionViewWidth,  height: themeCollectionViewHeight), collectionViewLayout: UICollectionViewFlowLayout())
    ...
    ///   - kind: 重用View的种类
    /// - Returns: 返回重用Header
    private func fetchRSFirstHeaderCollectionReusableView(indexPath: IndexPath, kind: String) -> RSHeaderCollectionReusableView {
        let headerView: RSFirstHeaderCollectionReusableView = self.themeCollectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: firstSectionheaderReuseIdentifier, for: indexPath) as! RSFirstHeaderCollectionReusableView
    ...
    ///   - kind: kind
    /// - Returns: headerView
    private func fetchRSHeaderCollectionReusableView(indexPath: IndexPath, kind: String) -> RSHeaderCollectionReusableView {
        let headerView: RSHeaderCollectionReusableView = self.themeCollectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: headerReuseIdentifier, for: indexPath) as! RSHeaderCollectionReusableView
    ...
    private func fetchRSThemeCollectionViewCell(indexPath: IndexPath ) -> RSThemeCollectionViewCell {
        let cell: RSThemeCollectionViewCell = self.themeCollectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as! RSThemeCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if indexPath.section == 0 {
            return fetchRSFirstHeaderCollectionReusableView(indexPath: indexPath, kind: kind)
        }
        return fetchRSHeaderCollectionReusableView(indexPath: indexPath, kind: kind)
    }
