...
class RSMenuView: UIView, UICollectionViewDataSource {
    private var menueCollectionView: RSMenuCollectionView!      //存放菜单的CollectionView
    private var dataSource: DataSourceType!
    private var tapSelectThemeClosure: TapSelectThemeClosure!       //点击加号所执行的回调方法，将该事件回调到VC中，由VC显示操作菜单的页面
    private var didSelectItmeClosure: RSDidSelectItemClosureType!   //点击菜单的回调
    ...
    private func addMenuCollectionView() {
        self.menueCollectionView = RSMenuCollectionView(frame: getMenueCollectionViewFrame(),
                                                        data: dataSource[0])
    ...
    func fetchCEThemeCollectionViewCell(indexPath: IndexPath ) -> RSMenuCollectionViewCell {
        let cell: RSMenuCollectionViewCell = self.menueCollectionView.dequeueReusableCell(withReuseIdentifier: MenuCellReuseIdentifier, for: indexPath) as! RSMenuCollectionViewCell
